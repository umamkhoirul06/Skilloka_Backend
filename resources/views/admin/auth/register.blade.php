            ...
            <div class="hidden md:flex flex-col justify-between p-10 lg:p-12 border-r border-white/10 relative">
                <div>
                    ...
                    <p class="text-gray-400 text-sm leading-relaxed mb-8">
                        Lengkapi profil lembaga Anda untuk bergabung dengan ekosistem Skilloka. Kelola kelas, siswa, dan
                        sertifikasi dalam satu sentuhan.
                    </p>

                    <div class="space-y-5">
                        <div class="flex items-center">
                            <div
                                class="feature-icon w-10 h-10 rounded-lg flex items-center justify-center mr-4 flex-shrink-0">
                ...
                @if($errors->any())
                    <div class="mb-5 p-3 rounded-xl bg-red-500/10 border border-red-500/20">
                        <div class="flex items-start">
                            <svg class="w-4 h-4 text-red-400 mr-2 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="form-step active" id="step-1">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="text" name="name"
                                    value="{{ old('name') }}" placeholder="John Doe" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="label-sm">Email Admin</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="email"
                                    name="email" value="{{ old('email') }}" placeholder="admin@lpk.com" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="label-sm">No WhatsApp</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="text"
                                    name="phone" value="{{ old('phone') }}" placeholder="08xxxxxxxxxx" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="label-sm">Password</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="password"
                        ...
                        <div class="mt-8">
                            <button type="button" onclick="nextStep(2)"
                                class="btn-primary w-full py-3 rounded-xl text-white text-sm font-semibold shadow-lg">Selanjutnya
                    ...
                    <div class="form-step" id="step-2">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="text"
                                    name="lpk_name" value="{{ old('lpk_name') }}" placeholder="LPK Skilloka" required>
                            </div>
                            <div class="col-span-2">
                                <label class="label-sm">Nama Legal / Yayasan</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="text"
                                    name="legal_name" value="{{ old('legal_name') }}" placeholder="Yayasan Edukasi..."
                                    required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="label-sm">Nomor NIB</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="text" name="nib"
                                    value="{{ old('nib') }}" placeholder="NIB LPK" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label class="label-sm">Email Resmi LPK</label>
                                <input class="input-field w-full px-4 py-2.5 rounded-xl text-sm" type="email"
                                    name="lpk_email" value="{{ old('lpk_email') }}" placeholder="info@lpk.com">
                            </div>
                        </div>

                        <div class="flex gap-3 mt-8">
                            <button type="button" onclick="prevStep(1)"
                                class="btn-secondary px-5 py-3 rounded-xl text-sm font-semibold">Kembali</button>
                            <button type="button" onclick="nextStep(3)"
                                class="btn-primary flex-1 py-3 rounded-xl text-white text-sm font-semibold shadow-lg">Selanjutnya
                                <i class="fas fa-arrow-right ml-1"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="form-step" id="step-3">
                        <div class="grid gap-3">
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input class="input-field w-full px-4 py-2 rounded-xl text-sm" type="text"
                                        name="city" value="{{ old('city') }}" placeholder="Indramayu" required>
                                </div>
                                <div>
                                    <label class="label-sm">Instagram</label>
                                    <input class="input-field w-full px-4 py-2 rounded-xl text-sm" type="text"
                                        name="instagram" value="{{ old('instagram') }}" placeholder="@skilloka">
                                </div>
                            </div>

                            <div>
                                <label class="label-sm">Alamat Lengkap</label>
                                <textarea class="input-field w-full px-4 py-2 rounded-xl text-sm h-16" name="address"
                                    placeholder="Alamat lengkap..." required>{{ old('address') }}</textarea>
                            </div>

                            <div>
                                <label class="label-sm">Deskripsi & Fasilitas</label>
                                <textarea class="input-field w-full px-4 py-2 rounded-xl text-sm h-16"
                                    name="description" placeholder="Ceritakan program dan fasilitas Anda..."
                                    required>{{ old('description') }}</textarea>
                            </div>

                            <!-- Hidden facility input mapping since we combine them visually to save space -->
                            <input type="hidden" name="facilities"
                                value="{{ old('facilities', 'Lihat di deskripsi') }}">

                            <div class="flex items-start mt-2">
                                <input type="checkbox" required
                                    class="mt-0.5 rounded border-gray-600 bg-gray-700 text-purple-600 cursor-pointer">
                        ...
                        <div class="flex gap-3 mt-6">
                            <button type="button" onclick="prevStep(2)"
                                class="btn-secondary px-5 py-3 rounded-xl text-sm font-semibold">Kembali</button>
                            <button type="submit"
                                class="btn-primary flex-1 py-3 rounded-xl text-white text-sm font-semibold shadow-lg">Selesaikan
                                Pendaftaran</button>
                        </div>
                    </div>
