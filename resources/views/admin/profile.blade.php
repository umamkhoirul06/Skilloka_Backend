...
@section('content')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div
                            class="md:w-1/3 bg-gray-50 p-8 border-b md:border-b-0 md:border-r border-gray-200 flex flex-col items-center justify-start text-center">
                            ...
                            <div class="relative mb-6">
                                {{-- Logo Container --}}
                                <div
                                    class="w-40 h-40 rounded-full shadow-md border-4 border-white overflow-hidden bg-white">
                                    @php
                                        $defaultLogo = 'https://ui-avatars.com/api/?name=' . urlencode($lpk->name ?? 'L') . '&color=1e3a8a&background=e0e7ff&size=200';
                                        // Memastikan logo ter-load dengan aman
                                        $logoUrl = ($lpk && $lpk->logo) ? asset('storage/' . $lpk->logo) : $defaultLogo;
                                    @endphp
                                    <img id="logoPreview" src="{{ $logoUrl }}" alt="Logo LPK"
                            ...
                            <div class="w-full max-w-xs">
                                <label for="logoInput"
                                    class="flex items-center justify-center w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl shadow-sm text-sm font-bold text-gray-700 hover:bg-gray-100 cursor-pointer transition-colors">
                        ...
                        <div class="md:w-2/3 p-8 lg:p-10">
                            <h3 class="text-lg font-bold text-gray-900 mb-6 border-b border-gray-100 pb-2">Informasi Dasar
                            </h3>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    ...
                                    {{-- Nomor Telepon --}}
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-900 mb-2">Nomor Telepon / WA
                                            (Pribadi)</label>
                                ...
                                <h3 class="text-lg font-bold text-gray-900 mb-4 mt-8 border-b border-gray-100 pb-2">
                                    Informasi Lembaga</h3>

                                {{-- Nama LPK (Readonly) --}}
                                <div>
                                    <label class="block text-sm font-semibold text-gray-900 mb-2">Lembaga Pelatihan
                                        (LPK)</label>
                                    <div class="relative">
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">Nama lembaga terkunci. Silakan hubungi Super Admin
                                        untuk mengubah nama.</p>
                                </div>

                                {{-- Legal Name --}}
                                <div class="mt-6">
                                    <label class="block text-sm font-semibold text-gray-900 mb-2">Nama Legal LPK /
                                        Perusahaan</label>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
                                    {{-- Phone LPK --}}
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-900 mb-2">Nomor Telepon
                                            Dasar</label>
                                        <input type="text" name="phone_lpk"
                                            value="{{ old('phone_lpk', $lpk->phone ?? '') }}"
                                            class="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow">
                                    </div>

                                    {{-- WA Number --}}
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-900 mb-2">Nomor WhatsApp
                                            LPK</label>
                                        <input type="text" name="wa_number"
                                            value="{{ old('wa_number', $lpk->wa_number ?? '') }}"
                                            class="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow">
                                    </div>
                                </div>

                                {{-- Fasilitas LPK (DIPERBAIKI DENGAN NORMALISASI) --}}
                                <div class="mt-6">
                                    ...
                                    @php
                                        ...
                                        // 🔥 FIX: Mencegah error jika data di database berupa string JSON
                                        $rawFacilities = $lpk->facilities ?? [];
                                        $savedFacilities = is_string($rawFacilities)
                                            ? (json_decode($rawFacilities, true) ?: [])
                                            : (is_array($rawFacilities) ? $rawFacilities : []);

                                        $selectedFacilities = old('facilities', $savedFacilities);
                                    @endphp
                                    <div class="flex flex-wrap gap-4">
                                        @foreach($availableFacilities as $facility)
                                            <label
                                                class="flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors">
                                                <input type="checkbox" name="facilities[]" value="{{ $facility }}" {{ in_array($facility, $selectedFacilities) ? 'checked' : '' }}
                                                    class="text-indigo-600 rounded focus:ring-indigo-500 border-gray-300">
                                ...
                                {{-- Gallery Images (DIPERBAIKI DENGAN NORMALISASI) --}}
                                <div class="mt-6">
                                    <label class="block text-sm font-semibold text-gray-900 mb-2">Galeri Foto LPK</label>
                                    @php
                                        // 🔥 FIX: Mencegah error jika data images berupa string JSON
                                        $rawImages = $lpk->images ?? [];
                                        $galleryImages = is_string($rawImages)
                                        ...
                                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-4">
                                            ...
                                            <div class="flex flex-wrap gap-4">
                                                @foreach($galleryImages as $img)
                                                    <div
                                                        class="relative group rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                                                        <img src="{{ asset('storage/' . $img) }}" class="w-24 h-24 object-cover"
                                                            alt="LPK Gallery Image">
                                                    </div>
                                                @endforeach
                                            </div>
                                            <p class="text-xs text-gray-500 mt-3">Mengunggah gambar baru akan menambah koleksi
                                                galeri.</p>
                                        </div>
                                    @endif
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">Maksimal 5MB per gambar (JPG, JPEG, PNG). Bisa
                                        pilih multiple file.</p>
                                </div>

                                {{-- Kontak Informasi LPK (DIPERBAIKI) --}}
                                <div class="mt-6">
                                    <label class="block text-sm font-semibold text-gray-900 mb-2">Kontak LPK Publik
                                        Tambahan</label>
                                    @php
                                        $rawContact = $lpk->contact_info ?? '';
                                        $decodedContact = is_string($rawContact) ? (json_decode($rawContact, true) ?: $rawContact) : $rawContact;
                                        $contactValue = is_array($decodedContact) ? implode(', ', $decodedContact) : $decodedContact;
                                    @endphp
                                    <textarea name="contact_info" rows="2"
                                        placeholder="Contoh: Email lpk@gmail.com atau IG @lpk_anda"
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Kontak tambahan yang akan ditampilkan di halaman
                                        dashboard utama Anda.</p>
                                </div>
                    ...
                    <div class="bg-gray-50 px-8 py-5 border-t border-gray-200 flex justify-end gap-3">
                        <a href="{{ route('admin.dashboard') }}"
                            class="px-6 py-2.5 rounded-xl border border-gray-300 text-sm font-bold text-gray-700 bg-white hover:bg-gray-100 transition-colors">
                            Batal
                        </a>
                        <button type="submit" style="background-color: #0f172a; color: white;"
                            class="px-8 py-2.5 rounded-xl text-sm font-bold shadow-md hover:opacity-90 transition-all">
                            Simpan Perubahan
                        </button>
                    </div>
    ...
    {{-- Modal Cropper --}}
    <div id="cropperModal"
        class="fixed inset-0 z-50 hidden overflow-y-auto bg-black bg-opacity-75 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col">
            ...
            <div class="p-4 border-t bg-gray-50 flex justify-end gap-3">
                <button type="button" id="cancelCrop"
                    class="px-5 py-2 rounded-xl text-sm font-bold text-gray-700 bg-white border border-gray-300 hover:bg-gray-100">Batal</button>
                <button type="button" id="applyCrop" style="background-color: #0f172a; color: white;"
                    class="px-6 py-2 rounded-xl text-sm font-bold shadow-md hover:opacity-90">Terapkan (Crop)</button>
            </div>
        </div>
    </div>
